...
async function main(): Promise<void> {
  ...
  try {
    ...
    write([
      ...
      `    ${origin}/dashboard/loads       under "My loads", the CLAIMED order above — open the row and press`,
      `                                        "Assign a vehicle" in the detail drawer (on a phone, the card's`,
      `                                        "Yours · assign a vehicle" strip) to open the vehicle picker`,
      `                                        (GET /api/logistics-company/orders/<id>/dispatch-options).`,
      ...
      "                                        after which the control disappears from that row — the order is no",
      "                                        longer awaiting dispatch — so re-run this seed to put it back into",
      "                                        CLAIMED.",
