...
import type {
  ...
  AdminSellerRow,
  AdminSellerType,
} from "@/app/api/admin/users/sellers/route";
import {
  SuspendDialog,
  type SuspendTarget,
...
/**
 * The two kinds of account this table moderates side by side: an independent
...
/**
 * `/admin/users/sellers` — the supply side of the marketplace: independent
 * drivers and logistics companies in one moderated list, told apart by the Type
 * column. They live in two different tables (`DriverProfile`,
 * `LogisticsCompany`) but are the same thing to staff answering a complaint
 * about a delivery, so the endpoint merges them and this page never has to know
 * which table a row came from.
 *
 * Structurally a twin of the Clients tab, deliberately duplicated rather than
 * abstracted behind a shared table component: the two lists show different
 * columns and are likely to diverge further (vehicle and payout context belong
 * only here), and a premature shared shell would have to grow a flag per
 * difference.
 *
 * Moderation is narrow on purpose — read the account, suspend or unsuspend it.
...
export default function AdminSellersPage() {
  ...
  const [data, setData] = useState<AdminSellerListResponse | null>(null);
    ...
    async function load() {
      try {
        const response = await fetch(
          `/api/admin/users/sellers?${params.toString()}`,
        ...
        setData((await response.json()) as AdminSellerListResponse);
        setLoading(false);
      } catch {
        ...
        <Table>
          ...
          <TableBody>
            {error ? (
              ...
              items.map((seller: AdminSellerRow) => {
                const expanded = expandedUserId === seller.userId;
                    ...
                    <TableRow>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1.5">
                          ...
                          <Button
                            variant={
                              seller.isSuspended ? "outline" : "destructive"
                            }
                            size="sm"
                            onClick={() =>
                              setSuspendTarget({
